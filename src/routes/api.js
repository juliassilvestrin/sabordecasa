...
const { findRecipeSubstitutes } = require('../services/recipe-service');
...
router.post('/recipe', async (req, res) => {
  const { recipe, lang } = req.body;
  if (!recipe || typeof recipe !== 'string' || recipe.trim().length < 10) {
    return res.status(400).json({ status: 'error', message: 'Please paste a recipe with ingredients.' });
  }
  if (recipe.trim().length > 4000) {
    return res.status(400).json({ status: 'error', message: 'Recipe is too long. Please keep it under 4000 characters.' });
  }
  try {
    const result = await findRecipeSubstitutes(recipe.trim(), lang || 'en');
    return res.json(result);
  } catch (error) {
